...
use strict;
use warnings;
use autouse 'iMSCP::Stepper' => 'step';
use File::Basename;
use File::Find;
use iMSCP::Boolean;
...
sub loadConfig
{
    ...
    tie %::imscpConfig, 'iMSCP::Config', fileName => $newConffile, readonly => 1, temporary => TRUE;
    ...
    $::imscpConfig{'DIST_ID'} = lc iMSCP::LsbRelease->getInstance()->getId( TRUE );
    $::imscpConfig{'DIST_CODENAME'} = lc iMSCP::LsbRelease->getInstance()->getCodename( TRUE );
    $::imscpConfig{'DIST_RELEASE'} = iMSCP::LsbRelease->getInstance()->getRelease( TRUE, TRUE );
...
sub build
{
    ...
    my $rs = 0;
    $rs = _installPreRequiredPackages() unless iMSCP::Getopt->skipDistPackages;
    return $rs if $rs;

    my $dialog = iMSCP::Dialog->getInstance();

    if ( !iMSCP::Getopt->noninteractive && isStringInList( 'none', @{ iMSCP::Getopt->reconfigure } ) ) {
    ...
    my @steps = (
        ...
        ( iMSCP::Getopt->skipDistPackages ? () : [ \&_installDistroPackages, 'Installing distribution packages' ] ),
    ...
    my ( $nbStep, $nbSteps ) = ( 1, scalar @steps );
    for my $step ( @steps ) {
        $rs = step( @{ $step }, $nbSteps, $nbStep );

        if ( $rs ) {
            error( 'An error occurred while performing build steps' ) if $rs != 50;
            return $rs;
        }
...
sub install
{
    ...
    for ( 'imscp-backup-all', 'imscp-backup-imscp', 'imscp-dsk-quota', 'imscp-srv-traff', 'imscp-vrl-traff', 'awstats_updateall.pl',
        'imscp-disable-accounts', 'imscp'
    ) {
...
EOF
    ...
    return 50 if $dialog->boolean( <<"EOF", 'abort_by_default' );
...
sub _askInstallerMode
{
    ...
    my ( $rs, $mode ) = $dialog->select( <<'EOF', \%choices, 'auto' );
...
sub _buildLayout
{
    my $distLayout = "$FindBin::Bin/autoinstaller/Layout/" . iMSCP::LsbRelease->getInstance()->getId( TRUE ) . '.xml';
    my $defaultLayout = "$FindBin::Bin/autoinstaller/Layout/Debian.xml";
    _processXmlFile( -f $distLayout ? $distLayout : $defaultLayout );
}
...
{
    my $distDir = "$FindBin::Bin/configs/" . lc iMSCP::LsbRelease->getInstance()->getId( TRUE );
    my $defaulDir = "$FindBin::Bin/configs/debian";
    my $confDir = -f "$distDir/install.xml" ? $distDir : $defaulDir;

    local $CWD = $confDir;
    my $rs = _processXmlFile( 'install.xml' );
    return $rs if $rs;

    local $CWD = $defaulDir;
    while ( my $dentry = <*> ) {
        $confDir = -f "$distDir/$dentry/install.xml" ? "$distDir/$dentry" : ( -f "$defaulDir/$dentry" ? "$defaulDir/$dentry" : undef );
        next unless defined $confDir;
        local $CWD = $confDir;
        $rs = _processXmlFile( 'install.xml' );
        return $rs if $rs;
    }
...
sub _buildEngineFiles
{
    ...
    while ( my $dentry = <*> ) {
        next unless -f "$dentry/install.xml";
        local $CWD = $dentry;
        $rs = _processXmlFile( 'install.xml' );
        return $rs if $rs;
    }
...
sub _removeObsoleteFiles
{
    for my $dir ( "$::imscpConfig{'CACHE_DATA_DIR'}/addons",
        ...
        "$::imscpConfig{'CONF_DIR'}/clamav",
        "$::imscpConfig{'CONF_DIR'}/frontend",
        "$::imscpConfig{'CONF_DIR'}/pma",
        "$::imscpConfig{'CONF_DIR'}/rainloop",
        "$::imscpConfig{'CONF_DIR'}/rspamd",
    ...
    for my $file ( "$::imscpConfig{'CONF_DIR'}/apache/parts/domain_disabled_ssl.tpl",
        ...
        '/usr/local/sbin/maillogconvert.pl',
...
sub _processXmlFile
{
    ...
    my $data = $xml->XMLin( $file, VarAttr => 'export', NormaliseSpace => 2 );
...
{
    $distAdapter ||= do {
        $distAdapter = "autoinstaller::Adapter::@{ [ iMSCP::LsbRelease->getInstance()->getId( TRUE ) ] }Adapter";
