...
Note that PHP **7.2** cannot be selected yet for the control panel. However, it
can be selected for the customers.
...
Historically, the DNS server names were generated based on the clients domain
names. For instance:
...
where `domain.tld` was the customer domain.

The problem with this behavior is that it is difficult to be fully compliant
...
DNS server name based on client domains involve different IP addresses because
most of time, only one single PTR record can be set per IP address.

Another problem is the fact that for each DNS server name there is the
corresponding glue record to create. This is really a problem for the clients
providing their own domains, and which are unable to create them due to
insufficient knownledge. This is of course also a problem for ISP and IT
companies. Having to create glue records each time a new domain is added to
the control panel is really a pain.

That is why the installer now make possible to setup DNS server names and which
are used throughout all DNS zone files. This make also possible to enable
DNS dual stacking more easily. Bassically put, the features provided by the
[`10_named_global_ns.pl`(../contrib/Listeners/Named/10_named_global_ns.pl/)
listener file have been integrated in i-MSCP core.

When needed, the installer will ask you for the DNS servers IP addresses, and
you will be optionally able to set the names associated to these IP addresses
(reverse DNS). If you don't do so, the system will fallback to the historical
behavior.
...
The following A/AAAA DNS records were removed from the DNZ template files:
...
Simply put, your customers *MUST* access the FTP, IMAP/POP and SMTP servers
using the server hostname. Of course this necessarily mean that the server
...
1. Outgoing mail is sent through the primary server IP while the customer
domain IP addresses can be different
