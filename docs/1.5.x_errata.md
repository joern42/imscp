...
in mind that the i-MSCP networking configuration provider for Netplan  has not
been backported to the `1.5.x` Serie yet. This means that if you want use this
new i-MSCP version with Ubuntu >= 18.04 (Bionic Beaver), you need first
reinstall the `ifupdown` package and configure your network through the
