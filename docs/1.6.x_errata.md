...
The i-MSCP installer now make sure that all distribution packages are
up-to-date and that the pre-requisites are met before starting the
...
When changing a setting manually in the i-MSCP master configuration file e.g
`/etc/imscp/imscp.conf` file, and if the debug mode was not enabled prior that
change, you *MUST* not forget to reload the `imscp_panel` service, else your
modifications won't be taken into account due to resources caching.

Prior the 1.6.x Serie, a check on last modification of the the i-MSCP master
configuration file was made on every requests. That is not longer the case for
performance reasons.

#### Files location

The i-MSCP frontend files were moved from the `/var/www/imscp/gui` directory to
the `/var/www/imscp/frontend` directory (default layout)

#### Dependencies

The frontend dependencies are now installed through the
...
Dual stacking of IPv4 and IPv6 is now fully supported by iMSCP server
implementations. From now on, the resellers can assign more than one IP
addresse to their customers, and the customers can assign one or many IP
addresses to their domains and subdomains.
...
time by a daemon running in background (recommended)
- Via cron(8): Backend requests are looked-up and processed every minutes
through a cron task.
