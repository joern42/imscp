...
sub processDbTasks
{
    ...
    if ( %{ $rows } ) {
        newDebug( 'imscp_sw_mngr_backend' );
        ...
        for my $row ( values %{ $rows } ) {
            ...
            my ( $stdout, $stderr );
            execute( "perl $::imscpConfig{'BACKEND_ROOT_DIR'}/imscp-sw-mngr " . escapeShell( $pushString ), \$stdout, \$stderr ) == 0 or die(
    ...
    if ( %{ $rows } ) {
        newDebug( 'imscp_pkt_mngr_backend.log' );
        ...
        for my $row ( values %{ $rows } ) {
            ...
            my ( $stdout, $stderr );
            execute( "perl $::imscpConfig{'BACKEND_ROOT_DIR'}/imscp-pkt-mngr " . escapeShell( $pushstring ), \$stdout, \$stderr ) == 0 or die(
                $stderr || 'Unknown error'
            );
            debug( $stdout ) if length $stdout;
