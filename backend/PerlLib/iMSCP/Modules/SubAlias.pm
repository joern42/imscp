...
sub _loadEntityData
{
    ...
    my $row = $self->{'_dbh'}->selectrow_hashref(
        "
            SELECT t1.*, t2.alias_id, t2.alias_name, t2.external_mail, t3.domain_name AS user_home, t3.domain_id, t3.domain_admin_id,
                t3.domain_mailacc_limit, t3.domain_php, t3.domain_cgi, t3.web_folder_protection, t3.phpini_perm_config_level AS php_config_level,
                IFNULL(GROUP_CONCAT(t4.ip_number), '0.0.0.0') AS domain_ips, t5.private_key, t5.certificate, t5.ca_bundle, t5.allow_hsts,
                t5.hsts_max_age, t5.hsts_include_subdomains
            FROM subdomain_alias AS t1
            JOIN domain_aliases AS t2 USING(alias_id)
            JOIN domain AS t3 USING (domain_id)
            LEFT JOIN server_ips AS t4 ON(FIND_IN_SET(t4.ip_id, t1.subdomain_alias_ips) AND t4.ip_status = 'ok' )
            ...
            GROUP BY t1.subdomain_alias_id
        ",
        undef, $entityId
    );
