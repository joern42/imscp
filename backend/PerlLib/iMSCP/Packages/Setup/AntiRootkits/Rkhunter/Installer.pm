...
sub _addCronTask
{
    iMSCP::Servers::Cron->factory()->addTask( {
        ...
        COMMAND => 'nice -n 10 ionice -c2 -n5 '
            . "perl $::imscpConfig{'BACKEND_ROOT_DIR'}/PerlLib/iMSCP/Packages/Setup/AntiRootkits/Rkhunter/Cron.pl > /dev/null 2>&1"
    } );
}
...
sub _scheduleCheck
{
    ...
    my $rs = execute(
        "echo 'perl $::imscpConfig{'BACKEND_ROOT_DIR'}/PerlLib/iMSCP/Packages/Setup/AntiRootkits/Rkhunter/Cron.pl > /dev/null 2>&1' | at now + 25 minutes",
        \my $stdout,
        \my $stderr
    );
