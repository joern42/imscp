=head1 NAME
 ...
 Package::Setup::AntiRootkits::Rkhunter::Installer - i-MSCP Rkhunter package installer
...
package Package::Setup::AntiRootkits::Rkhunter::Installer;
...
sub _disableDebianConfig
{
    ...
    if ( -f "$main::imscpConfig{'LOGROTATE_CONF_DIR'}/rkhunter" ) {
        my $rs = iMSCP::File->new( filename => "$main::imscpConfig{'LOGROTATE_CONF_DIR'}/rkhunter" )->moveFile( '/etc/logrotate.d/rkhunter.disabled' );
        return $rs if $rs;
    }
...
sub _addCronTask
{
    Servers::cron->factory()->addTask( {
        TASKID  => 'Package::Setup::AntiRootkits::Rkhunter',
        MINUTE  => '@weekly',
        HOUR    => '',
        DAY     => '',
        MONTH   => '',
        ...
        COMMAND => 'nice -n 10 ionice -c2 -n5 '
            . "perl $main::imscpConfig{'ENGINE_ROOT_DIR'}/PerlLib/Package/Setup/AntiRootkits/Rkhunter/Cron.pl > /dev/null 2>&1"
    } );
}
...
sub _scheduleCheck
{
    ...
    $rs = execute(
        "echo 'perl $main::imscpConfig{'ENGINE_ROOT_DIR'}/PerlLib/Package/Setup/AntiRootkits/Rkhunter/Cron.pl > /dev/null 2>&1' | at now + 10 minutes",
        \my $stdout,
        \my $stderr
    );
