=head1 NAME
 ...
 Package::Setup::FrontEnd - i-MSCP FrontEnd package
...
package Package::Setup::FrontEnd;

use strict;
use warnings;
use Class::Autouse qw/ :nostat Package::Setup::FrontEnd::Installer Package::Setup::FrontEnd::Uninstaller /;
use File::Basename;
use File::Spec;
use iMSCP::Config;
use iMSCP::Debug;
...
sub registerSetupListeners
{
    ...
    Package::Setup::FrontEnd::Installer->getInstance()->registerSetupListeners( $eventManager );
}
...
sub install
{
    ...
    $rs ||= Package::Setup::FrontEnd::Installer->getInstance()->install();
...
sub dpkgPostInvokeTasks
{
    ...
    $rs ||= Package::Setup::FrontEnd::Installer->getInstance()->dpkgPostInvokeTasks();
...
sub uninstall
{
    ...
    $rs ||= Package::Setup::FrontEnd::Uninstaller->getInstance()->uninstall();
...
END
    {
        ...
        if ( defined $main::execmode ) {
            ...
            $? = Package::Setup::FrontEnd->getInstance()->restartNginx() if $main::execmode eq 'uninstaller';
            return;
        }

        my $self = Package::Setup::FrontEnd->getInstance();
        if ( $self->{'start'} ) {
            $? = $self->start();
        } elsif ( $self->{'restart'} ) {
            $? = $self->restart();
