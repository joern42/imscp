=head1 NAME
 ...
 Package::Setup::PhpMyAdmin::Installer - i-MSCP PhpMyAdmin package installer
...
package Package::Setup::PhpMyAdmin::Installer;
...
=item _init( )
 ...
 Return Package::Setup::PhpMyAdmin::Installer
...
sub _init
{
    ...
    $self->{'phpmyadmin'} = Package::Setup::PhpMyAdmin->getInstance();
...
{
    my $frontEnd = Package::Setup::FrontEnd->getInstance();
    $frontEnd->buildConfFile(
        "$::imscpConfig{'ENGINE_ROOT_DIR'}/PerlLib/Package/Setup/PhpMyAdmin/config/nginx/imscp_pma.nginx",
...
sub _setVersion
{
    ...
    unless ( defined $json ) {
        error( sprintf( "Couldn't read %s", "$::imscpConfig{'GUI_PUBLIC_DIR'}/tools/pma/composer.json" ));
        return 1;
    }
