...
sub process
{
    ...
    my @sql;
    if ( grep ( $self->{'status'} eq $_, 'toadd', 'tochange' ) ) {
        $rs = $self->add();
        @sql = (
    ...
    # On toadd and tochange actions, return 0 to avoid any failure on update when a customer's SSL certificate is
...
sub delete
{
    ...
    my $rs = iMSCP::File->new( filename => "$self->{'certsDir'}/$self->{'domain_name'}.pem" )->delFile();
...
sub _loadData
{
    ...
    local $self->{'dbh'}->{'RaiseError'} = TRUE;
    my $row = $self->{'dbh'}->selectrow_hashref( 'SELECT * FROM ssl_certs WHERE cert_id = ?', undef, $certificateId );
    $row or die( sprintf( 'Data not found for SSL certificate (ID %d)', $certificateId ));
    %{ $self } = ( %{ $self }, %{ $row } );

    if ( $self->{'domain_type'} eq 'dmn' ) {
        $row = $self->{'dbh'}->selectrow_hashref( 'SELECT domain_name FROM domain WHERE domain_id = ?', undef, $self->{'domain_id'} );
    ...
    unless ( $row ) {
        ...
        $self->{'dbh'}->do( 'DELETE FROM FROM ssl_certs WHERE cert_id = ?', undef, $certificateId );
    } else {
        %{ $self } = ( %{ $self }, %{ $row } );
    }
