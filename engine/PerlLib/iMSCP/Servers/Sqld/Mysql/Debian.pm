...
sub uninstall
{
    ...
    eval { $self->restart() if iMSCP::Service->getInstance()->hasService( 'mysql' ); };
    if ( $@ ) {
        error( $@ );
        return 1;
    }
...
EOF
        ...
        $rs = execute( "cat $dbSchemaFile | /usr/bin/mysql --defaults-extra-file=$mysqlConffile", \ my $stdout, \ my $stderr );
        debug( $stdout ) if $stdout;
...
sub _cleanup
{
    ...
    return 0 unless version->parse( $main::imscpOldConfig{'PluginApi'} ) < version->parse( '1.5.1' ) && -f "$self->{'cfgDir'}/mysql.old.data";
