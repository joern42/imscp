...
=item process( \%data, $tpl [, $emptyUnknownVars = FALSE ] )
 ...
 Param hashref \%data A hash of data where the keys are the pseudo-variable names and the values, the replacement values
 Param string $tpl The template content to be processed
 Param boolean $emptyUnknownVars Flag indicating whether unknown variables must be emptied
...
=item getBlocByRef( $beginTag, $eTag, \$tpl [, $iTags = false ] )
 ...
 param scalarref $tpl Reference to template content
 Param bool $iTags OPTIONAL Flag indicating whether or not begin and ending tag must be included in result
