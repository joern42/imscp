=head1 NAME
 ...
 iMSCP::Dialog - Display dialog boxes using DIALOG(1)
...
=item infobox( $text )
 ...
 Show info dialog box (same as msgbox but exit immediately)
...
=item inputbox( $text [, $default = '' ] )
 ...
 Return string|array Input string or an array containing both DIALOG(1) exit code and input string
...
=item passwordbox( $text [, $default = '' ])
 ...
 Param string $text Text to show
 Param string $default Default value
 Return string|array Password string or an array containing both DIALOG(1) exit code and input string
...
sub startGauge
{
    ...
    open $self->{'_gauge'}, '|-', $self->_findBin(), $self->_getCommonOptions(), '--gauge', $text, $self->{'lines'}, $self->{'columns'},
...
sub resetLabels
{
    @{ $_[0]->{'_opts'} }{qw/ ok-label yes-label no-label cancel-label extra-label /} = (
        'Ok', 'Yes', 'No', 'Cancel', undef
    );
}
...
sub _init
{
    ...
    @{ $self->{'_opts'} }{qw/
        clear defaultno no-cancel extra-button cr-wrap no-collapse insecure no-tags
        ok-label yes-label no-label cancel-label extra-label title backtitle
    /} = (
...
sub _findBin
{
    CORE::state $bin = iMSCP::ProgramFinder::find( 'dialog' ) or die( "Couldn't find dialog program" );
    $bin;
}
...
sub _getCommonOptions
{
    ...
    ( map { defined $self->{'_opts'}->{$_} ? ( '--' . $_, ( $self->{'_opts'}->{$_} eq '' ? () : $self->{'_opts'}->{$_} ) ) : () }
        keys %{ $self->{'_opts'} }
    ), '--colors';
}
...
=item _execute( $boxType, @boxOptions )
 ...
 Execute DIALOG(1) commands

 Param string $boxType Box type
 Param list @boxOptions Box options 
 Return string|array Dialog output or array containing both DIALOG(1) exit code and dialog output
...
sub _execute
{
    ...
    if ( iMSCP::Getopt->noprompt ) {
        unless ( grep ( $boxType eq $_, 'infobox', 'msgbox' ) ) {
            iMSCP::Debug::error( sprintf( 'Failed dialog: %s', $boxOptions[0] ));
            exit 5
        }
    ...
    my $ret = execute( [ $self->_findBin(), $self->_getCommonOptions(), "--$boxType", @boxOptions ], undef, \my $output );
