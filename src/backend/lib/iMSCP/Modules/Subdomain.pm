...
sub process
{
    ...
    my @sql;
    if ( grep ( $self->{'subdomain_status'} eq $_, 'toadd', 'tochange', 'toenable' ) ) {
        $rs = $self->add();
        @sql = (
            'UPDATE subdomain SET subdomain_status = ? WHERE subdomain_id = ?',
            undef, $rs ? getLastError( 'error' ) || 'Unknown error' : 'ok', $subdomainId
        );
    } elsif ( $self->{'subdomain_status'} eq 'todelete' ) {
        $rs = $self->delete();
        @sql = $rs ? ( 'UPDATE subdomain SET subdomain_status = ? WHERE subdomain_id = ?',
            undef, getLastError( 'error' ) || 'Unknown error', $subdomainId
        ) : (
        ...
        @sql = (
            'UPDATE subdomain SET subdomain_status = ? WHERE subdomain_id = ?',
            undef, $rs ? getLastError( 'error' ) || 'Unknown error' : 'disabled', $subdomainId
        );
