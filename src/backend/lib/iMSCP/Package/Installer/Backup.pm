...
sub install
{
    ...
    my $binDir = "$::imscpConfig{'SHARE_DIR'}/iMSCP/Package//Installer/Backup/bin";
    my $cronServer = iMSCP::Server::cron->factory();

    if ( $self->{'config'}->{'BACKUP_IMSCP'} eq 'yes' ) {
        # Cron task for backup of i-MSCP configuration files and database
        my $rs = $cronServer->addTask( {
            TASKID  => __PACKAGE__ . ' - imscp backups',
            MINUTE  => '@daily',
            COMMAND => "$binDir/imscp-backup-imscp > $::imscpConfig{'LOG_DIR'}/imscp-backup-imscp.log 2>&1"
        } );

        # Cron task for deletion of outdated backups
        $rs ||= $cronServer->addTask( {
            TASKID  => __PACKAGE__ . ' - imscp backups (cleanup)',
            MINUTE  => '@weekly',
            COMMAND => "find $self->{'config'}->{'BACKUP_DIR'} -type f -mtime +7 -exec rm -- {} \+"
        } );
        return $rs if $rs;
    }

    if ( $self->{'config'}->{'BACKUP_CLIENTS'} eq 'yes' ) {
        # Cron task for backup of client data
        my $rs = $cronServer->addTask( {
            TASKID  => __PACKAGE__ . ' - client backups',
            MINUTE  => $self->{'config'}->{'BACKUP_CLIENTS_MINUTE'} || 40,
            HOUR    => $self->{'config'}->{'BACKUP_CLIENTS_HOUR'} || 23,
            COMMAND => "nice -n 10 ionice -c2 -n5 perl $binDir/imscp-backup-client > $::imscpConfig{'LOG_DIR'}/imscp-client-backup.log 2>&1"
        } );
        return $rs if $rs;
    }
    ...
    $cronServer->addTask( {
        TASKID  => __PACKAGE__ . ' - server backups (cleanup)',
        MINUTE  => '@weekly',
...
sub uninstall
{
    ...
    my $rs = $cronServer->deleteTask( { TASKID => __PACKAGE__ . ' - imscp backups' } );
    $rs ||= $cronServer->deleteTask( { TASKID => __PACKAGE__ . ' - imscp backups (cleanup)' } );
    $rs ||= $cronServer->deleteTask( { TASKID => __PACKAGE__ . ' - client backups' } );
    $rs ||= $cronServer->deleteTask( { TASKID => __PACKAGE__ . ' - server backups (cleanup)' } );
}
...
sub _init
{
    ...
    my $configProvider = iMSCP::Provider::Config::iMSCP->new(
        DISTRIBUTION_FILE => "$::imscpConfig{'SHARE_DIR'}/iMSCP/Package//Installer/Backup/config/Backup.conf.dist",
        PRODUCTION_FILE   => "$::imscpConfig{'CONF_DIR'}/Backup.conf"
    );
...
sub _askForClientsBackup
{
    ...
    my $value = ::setupGetQuestion( 'BACKUP_CLIENTS' );
...
EOF
    ...
    ::setupSetQuestion( 'BACKUP_CLIENTS', $value );
    0;
}
