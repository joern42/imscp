...
use iMSCP::Boolean;
use iMSCP::Config;
use CiMSCP::rypt qw/ ALNUM randomStr /;
use iMSCP::Debug qw/ debug error getMessageByType /;
use iMSCP::InputValidation qw/ isOneOfStringsInList isStringInList isValidPassword /;
...
sub _installFiles
{
    ...
    iMSCP::Dir->new( dirname => "$::imscpConfig{'SHARE_DIR'}/iMSCP/Package/Installer/PostfixAddon/Rspamd/config" )->copy( '/' );
}
...
sub _askForModules
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'antispam', 'all' ] )
        ...
        ( my $rs, $selectedModules ) = $dialog->multiselect(
...
sub _askForWebUIPassword
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'antispam', 'alternatives', 'all' ] )
        ...
        do {
            ( my $rs, $password ) = $dialog->string( <<"EOF", randomStr( 16, ALNUM ));
$iMSCP::InputValidation::lastValidationError
...
sub _setupCronTaskForSpamLearning
{
    ...
    my $mtaConfig = iMSCP::Server::mta->factory()->{'config'};
