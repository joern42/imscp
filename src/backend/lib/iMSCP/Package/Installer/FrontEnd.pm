...
sub addUser
{
    ...
    iMSCP::SystemUser->new( username => $::imscpConfig{'USER_PREFIX'} . $::imscpConfig{'USER_MIN_UID'} )->addToGroup(
        $data->{'GROUP'}
    );
}
...
sub _askForMasterAdminCredentials
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'cp', 'cp_admin', 'cp_admin_credentials', 'all' ] )
        ...
        do {
            ...
            ( my $rs, $username ) = $dialog->string( <<"EOF", $username );
$iMSCP::InputValidation::lastValidationError
...
EOF
            ...
            if ( isValidUsername( $username ) ) {
                unless ( $freshInstall ) {
                    ...
                    if ( $row ) {
                        $iMSCP::InputValidation::lastValidationError = <<"EOF";
\\Z1This username is not available.\\Zn
EOF
                    }
                }
            }
        } while length $iMSCP::InputValidation::lastValidationError;
        ...
        do {
            ...
            ( my $rs, $password ) = $dialog->string( <<"EOF", $password );
$iMSCP::InputValidation::lastValidationError
...
EOF
    ...
    ::setupSetQuestion( 'MASTER_ADMIN_EMAIL', $email );
    0;
}
...
sub _askForControlPanelDomain
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'cp', 'cp_hostname', 'hostname', 'all' ] ) || !isValidDomain( $domain ) ) {
        ...
        do {
            ( my $rs, $domain ) = $dialog->string( <<"EOF", $domain, 'utf-8' );
$iMSCP::InputValidation::lastValidationError
...
sub _askForControlPanelSSL
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'cp', 'cp_ssl', 'ssl', 'all' ] )
        ...
        Q1:
        my $rs = $dialog->boolean( <<'EOF', $sslEnabled eq 'no', TRUE );
...
EOF
        ...
        Q2:
        $rs = $dialog->boolean( <<"EOF", $selfSignedCertificate eq 'no', TRUE );
...
EOF
        ...
        unless ( $rs ) {
            Q3:
            do {
                ( $rs, $privateKeyPath ) = $dialog->string( <<"EOF", $privateKeyPath );
$msg
...
EOF
            ...
            Q4:
            ( $rs, $passphrase ) = $dialog->password( <<'EOF', $passphrase );
...
EOF
            ...
            Q5:
            $rs = $dialog->boolean( <<'EOF', FALSE, TRUE );
...
EOF
            ...
            Q6:
            do {
                ( $rs, $caBundlePath ) = $dialog->string( <<"EOF", $caBundlePath );
$msg
...
EOF
            ...
            Q7:
            do {
                ( $rs, $certificatePath ) = $dialog->string( <<"EOF", $certificatePath );
$msg
...
sub _askForControlPanelHttpPorts
{
    ...
    ( my $rs, $iMSCP::InputValidation::lastValidationError ) = ( 0, '' );
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'cp', 'cp_ports', 'all' ] )
        ...
        Q1:
        do {
            ( $rs, $httpPort ) = $dialog->string( <<"EOF", $httpPort ? $httpPort : 8880 );
$iMSCP::InputValidation::lastValidationError
...
EOF
        ...
        if ( $ssl eq 'yes' ) {
            do {
                ( $rs, $httpsPort ) = $dialog->string( <<"EOF", $httpsPort ? $httpsPort : 8443 );
$iMSCP::InputValidation::lastValidationError
...
sub _setupSsl
{
    ...
    if ( ::setupGetQuestion( 'PANEL_SSL_SELFSIGNED_CERTIFICATE' ) eq 'yes' ) {
        return iMSCP::OpenSSL->new(
            ...
            email       => ::setupGetQuestion( 'MASTER_ADMIN_EMAIL' )
        } );
    }
...
sub _addMasterWebUser
{
    ...
    my $rs = eval {
        ...
        $rs = iMSCP::SystemUser->new( username => $panelUserGroup )->addToGroup( iMSCP::Server::mta->factory()->{'config'}->{'MTA_MAILBOX_GID_NAME'} );
...
sub _buildPhpConfig
{
    ...
    $rs = $self->buildConfFile(
        "$self->{'cfgDir'}/php-fpm.conf",
        {
            ...
            OPENSSL_CNF               => $::imscpConfig{'OPENSSL_CNF'},
            CA_BUNDLE                 => $::imscpConfig{'CA_BUNDLE'},
            ...
            MTA_VIRTUAL_MAIL_DIR      => iMSCP::Server::mta->factory()->{'config'}->{'MTA_VIRTUAL_MAIL_DIR'},
...
sub _addDnsRecord
{
    ...
    $rs ||= iMSCP::Server::named->factory()->addSub( {
...
sub _deconfigurePHP
{
    ...
    for my $file (
        '/etc/default/imscp_panel', '/etc/tmpfiles.d/imscp_panel.conf', '/etc/logrotate.d/imscp_panel', '/usr/local/sbin/imscp_panel',
...
{
    my $rs = iMSCP::SystemUser->new( force => 'yes' )->delSystemUser( $::imscpConfig{'USER_PREFIX'} . $::imscpConfig{'USER_MIN_UID'} );
    $rs ||= iMSCP::SystemGroup->getInstance()->delSystemGroup( $::imscpConfig{'USER_PREFIX'} . $::imscpConfig{'USER_MIN_UID'} );
}
