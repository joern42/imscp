...
sub _disableDebianConfig
{
    ...
    if ( -f '/etc/logrotate.drkhunter' ) {
        my $rs = iMSCP::File->new( filename => '/etc/logrotate.d/rkhunter' )->moveFile( '/etc/logrotate.d/rkhunter.disabled' );
        return $rs if $rs;
    }
...
sub _addCronTask
{
    ...
    iMSCP::Server::cron->factory()->addTask( {
        ...
        MINUTE  => '@weekly',
        USER    => $::imscpConfig{'ROOT_USER'},
        COMMAND => "perl $::imscpConfig{'SHARE_DIR'}/iMSCP/Package/Installer/AntiRootkits/Rkhunter/bin/imscp-rkhunter > /dev/null 2>&1"
    } );
}

=item _deleteCronTask( )
...
sub _scheduleCheck
{
    ...
    $rs = execute(
        "echo 'perl $::imscpConfig{'SHARE_DIR'}/iMSCP/Package/Installer/AntiRootkits/Rkhunter/bin/imscp-rkhunter > /dev/null 2>&1 "
            .'| at now + 20 minutes',
        \my $stdout,
        \my $stderr
    );
...
sub _restoreDebianConfig
{
    ...
    if ( -f '/etc/logrotate.d/rkhunter.disabled' ) {
        my $rs = iMSCP::File->new( filename => '/etc/logrotate.d/rkhunter.disabled' )->moveFile( '/etc/logrotate.d/rkhunter' );
        return $rs if $rs;
    }
