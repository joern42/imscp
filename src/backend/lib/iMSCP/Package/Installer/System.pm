...
use iMSCP::InputValidation qw/ isOneOfStringsInList isStringNotInList isValidIpAddr isValidHostname isValidIpAddr isValidTimezone /;
...
sub preinstall
{
    ...
    if ( -f '/etc/sysctl.d/imscp.conf' ) {
        ...
        execute( "sysctl -p /etc/sysctl.d/imscp.conf", \my $stdout, \my $stderr );
    }
...
sub _askForServerPrimaryIP
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'system', 'system_primary_ip', 'alternatives', 'all' ] )
        ...
        do {
            ...
            ( my $rs, $lanIP ) = $dialog->select( <<"EOF", \%choices, ( grep ( $lanIP eq $_, @ipList ) )[0] || $ipList[0] );
...
EOF
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'system', 'system_primary_ip', 'alternatives', 'all' ] ) || !isValidIpAddr( $wanIP ) ) {
        do {
            ...
            ( my $rs, $wanIP ) = $dialog->string( <<"EOF", $wanIP );
$iMSCP::InputValidation::lastValidationError
...
EOF
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'system', 'system_primary_ip', 'all' ] ) ) {
        my $rs = $dialog->boolean( <<"EOF", TRUE, TRUE );
...
sub _askForServerTimezone
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'system', 'system_timezone', 'alternatives', 'all' ] ) || !isValidTimezone( $value ) ) {
        do {
            ( my $rs, $value ) = $dialog->string( <<"EOF", $value || DateTime::TimeZone->new( name => 'local' )->name());
$iMSCP::InputValidation::lastValidationError
...
sub _createDnsZone
{
    ...
    my $named = iMSCP::Server::named::factory();
    my $rs ||= $named->addDmn( {
        DOMAIN_NAME  => $domain,
