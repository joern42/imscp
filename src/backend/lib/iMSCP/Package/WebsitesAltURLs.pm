...
use strict;
use warnings;
use autouse 'iMSCP::InputValidation' => qw/ isOneOfStringsInList isStringInList /;
use Class::Autouse qw/ :nostat iMSCP::Server::httpd iMSCP::Server::named /;
use iMSCP::Boolean;
...
sub _init
{
    ...
    my $configProvider = iMSCP::Provider::Config::iMSCP->new(
        DISTRIBUTION_FILE => "$::imscpConfig{'SHARE_DIR'}/iMSCP/Package/Installer/Backup/config/WebsitesAltURLs.conf.dist",
        PRODUCTION_FILE   => "$::imscpConfig{'CONF_DIR'}/WebsitesAltURLs.conf"
    );

    $self->{'config'} = $configProvider->( $configProvider );
    $self->{'named'} = iMSCP::Server::named->factory();
    $self->{'httpd'} = iMSCP::Server::httpd->factory();
