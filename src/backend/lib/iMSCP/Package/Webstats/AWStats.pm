...
use Class::Autouse qw/ :nostat iMSCP::DistPackageManager iMSCP::Server::httpd /;
use iMSCP::Boolean;
...
sub addUser
{
    ...
    my $rs = $file->save();
    iMSCP::Server::httpd->factory()->{'restart'} = TRUE unless $rs;
    $rs;
}
...
sub _createCacheDir
{
    ...
    iMSCP::Dir->new( dirname => '/var/cache/awstats' )->make( {
        user           => iMSCP::Server::httpd->factory()->getRunningUser(),
        group          => iMSCP::Server::httpd->factory()->getRunningGroup(),
        mode           => 0750,
...
sub _setupApache2
{
    ...
    my $rs = $file->save();
    $rs ||= $file->owner( $::imscpConfig{'ROOT_USER'}, $httpd->getRunningGroup());
    $rs ||= $file->mode( 0640 );
    return $rs if $rs;

    # Enable required Apache2 modules
    $rs = $httpd->enableModules( 'rewrite', 'authn_core', 'authn_basic', 'authn_socache', 'proxy', 'proxy_http' );
    return $rs if $rs;

    # Create Apache2 vhost
    $rs = $httpd->buildConfFile( "$::imscpConfig{'SHARE_DIR'}/iMSCP/Package/Webstats/AWStats/templates/01_awstats.conf" );
    $rs ||= $httpd->enableSites( '01_awstats.conf' );
}
...
sub _cleanup
{
    ...
    while ( my $dentry = <$::imscpConfig{'SRV_DIR'}/*/statistics> ) {
...
sub _createConfig
{
    ...
    my $fileC = iMSCP::File->new( filename => "$::imscpConfig{'SHARE_DIR'}/iMSCP/Package/Webstats/AWStats/templates/awstats.conf.tpl" )->get();
    return 1 unless defined $fileC;
    ...
    processByRef(
        {
            ...
            HOST_ALIASES => iMSCP::Server::httpd->factory()->getData()->{'SERVER_ALIASES'}
        },
        \$fileC
    );
...
sub _deleteFiles
{
    ...
    my $rs = iMSCP::Server::httpd->factory()->disableSites( '01_awstats.conf' );
    return $rs if $rs;
