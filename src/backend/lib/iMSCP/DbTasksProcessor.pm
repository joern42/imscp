...
sub processDbTasks
{
    ...
    my $rows = $self->{'rdbh'}->selectall_hashref(
        "
    ...
    if ( %{ $rows } ) {
        ...
        for ( values %{ $rows } ) {
            ...
            my ( $stdout, $stderr );
            execute( "perl $::imscpConfig{'SBIN_DIR'}/imscp-sw-mngr " . escapeShell( $pushString ), \$stdout, \$stderr ) == 0 or die(
    ...
    $rows = $self->{'rdbh'}->selectall_hashref(
        "
    ...
    if ( %{ $rows } ) {
        ...
        for ( values %{ $rows } ) {
            ...
            my ( $stdout, $stderr );
            execute( "perl $::imscpConfig{'SBIN_DIR'}/imscp-pkt-mngr " . escapeShell( $pushstring ), \$stdout, \$stderr ) == 0 or die(
                $stderr || 'Unknown error'
            );
            debug( $stdout ) if $stdout;
...
sub _init
{
    ...
    $self->{'mode'} = iMSCP::Getopt->context();
...
=item _processModuleDbTasks( $module, $sql [, $perItemLogFile = FALSE ] )
 ...
 Return boolean TRUE if at least one item has been processed, FALSE if no item has been processed, die on failure
...
sub _processModuleDbTasks
{
    ...
    eval {
        ...
        my ( $nStep, $rs ) = ( 0, 0 );
        my $needStepper = grep ( $self->{'mode'} eq $_, ( 'installer', 'uninstaller' ) );
...
sub _processModuleTasks
{
    ...
    local $self->{'rdbh'}->{'RaiseError'} = FALSE;
