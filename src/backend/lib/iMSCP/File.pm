...
=item get( )
 ...
 to the in memory file content.
 
 Note that file content is read once. Subsequent calls return shallow
 copy of in memory content.

 Return string Shallow copy of loaded file's content, croak or die on failure
...
sub get
{
    ...
    local $/;
    $self->{'file_content'} = scalar readline $fh;
    close( $fh );
    $self->{'file_content'};
}
...
=item getAsRef( $skipLoad = FALSE )
 ...
 Return scalar reference poiting to in memory file content
 ...
 Return scalarref Reference to in memory file content, croak or die on failure
...
=item save( [ $umask = UMASK(2) ])
 ...
 Return self, croak or die on failure
...
sub owner
{
    ...
    my ( $uid ) = defined $owner
        ? ( $owner =~ /^(?:-1|\d+)$/ ? $owner : getpwnam( $owner ) // croak( sprintf( "Couldn't find user '%s'", $owner )) ) : -1;
    my ( $gid ) = defined $group
        ? ( $group =~ /^(?:-1|\d+)$/ ? $group : getgrnam( $group ) // croak( sprintf( "Couldn't find group '%s'", $group )) ) : -1;
...
sub copy
{
    ...
    endCopy:
    $ret or die( sprintf( "Failed to copy '%s' to '%s': %s", $self->{'filename'}, $dest, scalar getMessageByType( 'error', { remove => TRUE } )));
    $self;
}
...
=item _init( )
 ...
 Return self, croak if the 'filename' attribute is not set
