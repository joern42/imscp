=head1 NAME
 ...
 iMSCP::Server::named::bind - i-MSCP Bind9 Server implementation
...
package iMSCP::Server::named::bind;

use strict;
use warnings;
use autouse 'iMSCP::Rights' => 'setRights';
use Class::Autouse qw/ :nostat iMSCP::Server::named::bind::installer iMSCP::Server::named::bind::uninstaller /;
use File::Basename;
use iMSCP::Boolean;
use iMSCP::Config;
...
sub registerInstallerDialogs
{
    ...
    iMSCP::Server::named::bind::installer->getInstance()->registerInstallerDialogs( $dialogs );
}
...
sub install
{
    ...
    $rs ||= iMSCP::Server::named::bind::installer->getInstance()->install();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Server::named::bind::uninstaller->getInstance()->uninstall();
    return $rs if $rs;
...
=item _init( )
 ...
 Return iMSCP::Server::named::bind
...
sub _createZoneFile
{
    ...
    unless ( !defined $wDbFileC || iMSCP::Getopt->context() eq 'installer' ) {
        # Retrieve subdomain DNS records from current zone version and reinject
        ...
        # Retrieve cutstom DNS records from current zone version and reinject
        # them in new zone version
