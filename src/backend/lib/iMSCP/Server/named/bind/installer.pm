=head1 NAME
 ...
 iMSCP::Server::named::bind::installer - i-MSCP Bind9 Server implementation
...
package iMSCP::Server::named::bind::installer;
...
use iMSCP::InputValidation qw/ isOneOfStringsInList isStringNotInList isValidEmail isValidHostname /;
use iMSCP::Dir;
...
sub registerInstallerDialogs
{
    ...
    push @{ $dialogs },
        sub { $self->_askForDnsServerType( @_ ) },
        sub { $self->_askForDnsIPv6Support( @_ ) },
        sub { $self->_askForMasterDnsServer( @_ ) },
        sub { $self->_askForMasterDnsServerIpPolicy( @_ ) },
        sub { $self->_askForSlaveDnsServers( @_ ) },
        sub { $self->_askForLocalDnsResolver( @_ ) };
    0;
}
...
=item _init( )
 ...
 Return iMSCP::Server::named::bind::installer
...
sub _init
{
    ...
    $self->{'named'} = iMSCP::Server::named::bind->getInstance();
...
=item _askForDnsServerType( $dialog )
...
sub _askForDnsServerType
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'named_type', 'named', 'alternatives', 'all' ] )
        ...
        ( my $rs, $value ) = $dialog->select( <<'EOF', \%choices, ( grep ( $value eq $_, keys %choices ) )[0] || 'master' );
...
=item _askForDnsIPv6Support( $dialog )
...
sub _askForDnsIPv6Support
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'named_ipv6', 'named', 'alternatives', 'all' ] )
        ...
        my $rs = $dialog->boolean( <<'EOF', $value eq 'no', TRUE );
...
=item _askForMasterDnsServerIpPolicy( $dialog )
...
sub _askForMasterDnsServerIpPolicy
{
    my ( $self, $dialog ) = @_;
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'named_ips_policy', 'named', 'alternatives', 'all' ] )
        ...
        ( my $rs, $value ) = $dialog->boolean( <<"EOF", $value eq 'no', TRUE );
...
=item _askForMasterDnsServer( $dialog )
...
sub _askForMasterDnsServer
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'named_master', 'named', 'alternatives', 'all' ] )
        ...
        if ( $type eq 'master' ) {
            $rs = $dialog->boolean( <<'EOF', !@ips || $ips[0] eq 'none', TRUE );
...
EOF
        ...
        if ( $rs == 0 || $type eq 'slave' ) {
            ...
            Q2:
            do {
                ( $rs, my $ips ) = $dialog->string( <<"EOF", ::setupGetQuestion( 'BIND_TYPE' ) eq $self->{'oldBindType'} ? "@ips" : '' );
$msg
@{ [ 
...
EOF
            ...
            if ( $type eq 'master' && grep ( $_ ne 'none', @ips ) ) {
                ...
                for my $idx ( 0 .. $#ips ) {
                    push @dialogs, sub {
                        do {
                            ( $rs, $names[$idx] ) = $dialog->string( <<"EOF", $names[$idx] || _getReverseDNS( $ips[$idx] ));
$msg
...
EOF
                ...
                do {
                    $iMSCP::InputValidation::lastValidationError = '' unless length $email;

                    ( $rs, $email ) = $dialog->string( <<"EOF", $email && $email ne 'none' ? $email : "hostmaster\@$names[0]" );
$iMSCP::InputValidation::lastValidationError
...
=item _askForSlaveDnsServers( $dialog )
...
sub _askForSlaveDnsServers
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'named_slave', 'named', 'alternatives', 'all' ] )
        ...
        Q1:
        my $rs = $dialog->boolean( <<'EOF', !@ips || $ips[0] eq 'none', TRUE );
...
EOF
        ...
        if ( $rs == 0 ) {
            ...
            do {
                ( $rs, my $ips ) = $dialog->string( <<"EOF", "@ips" );
$msg
...
=item _askForLocalDnsResolver( $dialog )
...
sub _askForLocalDnsResolver
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'named_resolver', 'named', 'alternatives', 'all' ] )
        ...
        ( my $rs, $value ) = $dialog->boolean( <<'EOF', $value ne 'yes', TRUE );
...
sub _buildConf
{
    ...
    return 0 unless $self->{'config'}->{'BIND_LOCAL_CONF_FILE'};
...
sub _getReverseDNS
{
    ...
    eval {
        my $sockAddr = iMSCP::Net->getInstance()->getAddrVersion( $ipAddress ) eq 'ipv4'
            ? sockaddr_in( 0, inet_pton( AF_INET, $ipAddress )) : sockaddr_in6( 0, inet_pton( AF_INET6, $ipAddress ));
