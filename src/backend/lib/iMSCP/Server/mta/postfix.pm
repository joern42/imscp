=head1 NAME
 ...
 iMSCP::Server::mta::postfix - i-MSCP Postfix MTA server implementation
...
package iMSCP::Server::mta::postfix;

use strict;
use warnings;
use Class::Autouse qw/ :nostat iMSCP::Server::mta::postfix::installer iMSCP::Server::mta::postfix::uninstaller /;
use File::Basename;
use File::Temp;
use iMSCP::Boolean;
use iMSCP::Config;
...
sub preinstall
{
    ...
    $rs ||= $self->stop();
    $rs ||= $rs = iMSCP::Server::mta::postfix::installer->getInstance()->preinstall();
...
sub install
{
    ...
    $rs ||= iMSCP::Server::mta::postfix::installer->getInstance()->install();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Server::mta::postfix::uninstaller->getInstance()->uninstall();
...
sub addDmn
{
    ...
    # Do not list the system hostname in BOTH 'mydestination' and 'virtual_mailbox_domains'
    return 0 if $data->{'DOMAIN_NAME'} eq $self->{'server_hostname'};
...
sub addMail
{
    ...
    if ( $data->{'MAIL_CATCHALL'} ) {
        ...
        my $isMailAccount = index( $data->{'MAIL_TYPE'}, '_mail' ) != -1 && $data->{'DOMAIN_NAME'} ne $self->{'server_hostname'};
...
sub getTraffic
{
    ...
    if ( -f -s $logFile ) {
        ...
        open my $fh, '-|', "$::imscpConfig{'SBIN_DIR'}/vendor/maillogconvert.pl standard < $snapshotFH 2>/dev/null" or die(
...
=item postconf( $conffile, %params )
 ...
 Param hash %params A hash where each key is a Postfix parameter name and the value, a hashes describing in order:
  ...
  Usage example:
    ...
    iMSCP::Server::mta::postfix->getInstance(
        (
...
=item _init( )
 ...
 Return iMSCP::Server::mta::postfix
...
sub _init
{
    ...
    @{ $self }{qw/ start restart /} = ( FALSE, FALSE );
    $self->{'server_hostname'} = $::imscpConfig{'SERVER_HOSTNAME'};
    $self->{'cfgDir'} = "$::imscpConfig{'CONF_DIR'}/postfix";

    $self->_mergeConfig() if iMSCP::Getopt->context() eq 'installer' && -f "$self->{'cfgDir'}/postfix.data.dist";
    tie %{ $self->{'config'} },
        'iMSCP::Config',
