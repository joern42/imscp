=head1 NAME
 ...
 iMSCP::Server::sqld::mysql::installer - i-MSCP MySQL server installer implementation
...
package iMSCP::Server::sqld::mysql::installer;
...
=item _init( )
 ...
 Return iMSCP::Server::sqld::mysql:installer
...
sub _init
{
    ...
    $self->{'sqld'} = iMSCP::Server::sqld::mysql->getInstance();
...
sub _askForMasterSqlUser
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'sqld', 'alternatives', 'all' ] ) || !isNotEmpty( $hostname )
        ...
        do {
            ( $rs, $user ) = $dialog->string( <<"EOF", $user );
$iMSCP::InputValidation::lastValidationError
...
EOF
        ...
        do {
            ( $rs, $pwd ) = $dialog->string( <<"EOF", $pwd || randomStr( 16, ALNUM ));
$iMSCP::InputValidation::lastValidationError
...
sub _askForSqlUserHost
{
    ...
    if ( $::imscpConfig{'SQLD_PACKAGE'} ne 'iMSCP::Server::sqld::remote' ) {
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'sqld', 'alternatives', 'all' ] )
        ...
        do {
            ( my $rs, $hostname ) = $dialog->string( <<"EOF", idn_to_unicode( $hostname, 'utf-8' ));
$iMSCP::InputValidation::lastValidationError
...
sub _askForSqlDatabaseName
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'sqld', 'alternatives', 'all' ] )
        ...
        do {
            ( my $rs, $dbName ) = $dialog->string( <<"EOF", $dbName );
$iMSCP::InputValidation::lastValidationError
...
sub _askForSqlPrefixOrSuffix
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'sqld', 'alternatives', 'all' ] )
        ...
        ( my $rs, $value ) = $dialog->select( <<'EOF', \%choices, ( grep ( $value eq $_, keys %choices ) )[0] || 'none' );
...
sub _askSqlRootUser
{
    ...
    my $hostname = ::setupGetQuestion( 'DATABASE_HOST', $::imscpConfig{'SQLD_PACKAGE'} eq 'iMSCP::Server::sqld::remote' ? '' : 'localhost' );

    if ( $::imscpConfig{'SQLD_PACKAGE'} eq 'iMSCP::Server::sqld::remote' && isStringInList( $hostname, 'localhost', '127.0.0.1', '::1' ) ) {
        # Handle switch case (default value). Host cannot be one of above value
        # when using remote SQL server
        $hostname = '';
    }
    ...
    Q1:
    do {
        ( my $rs, $hostname ) = $dialog->string( <<"EOF", $hostname );
$iMSCP::InputValidation::lastValidationError
...
EOF
    ...
    Q2:
    do {
        ( my $rs, $port ) = $dialog->string( <<"EOF", $port );
$iMSCP::InputValidation::lastValidationError
...
EOF
    ...
    Q3:
    do {
        ( my $rs, $user ) = $dialog->string( <<"EOF", $user );
$iMSCP::InputValidation::lastValidationError
...
\\Z1Connection to SQL server has failed\\Zn
...
EOF
    ...
    if ( $::imscpConfig{'SQLD_PACKAGE'} eq 'iMSCP::Server::sqld::percona'
    ...
    if ( version->parse( "$self->{'config'}->{'SQLD_VERSION'}" ) >= version->parse( '5.7.4' )
        && $::imscpConfig{'SQLD_PACKAGE'} ne 'iMSCP::Server::sqld::mariadb'
    ) {
...
sub _updateServerConfig
{
    ...
    if ( !( $::imscpConfig{'SQLD_PACKAGE'} eq 'iMSCP::Server::sqld::mariadb'
...
sub _setupDatbase
{
    ...
    unless ( $self->_setupIsImscpDb( $dbName ) ) {
        ...
        $rs = ::setupImportSqlSchema( $self->{'dbh'}, "$::imscpConfig{'SHARE_DIR'}/database/database.sql" );
    ...
    $rs ||= execute(
        [
            'php',
            '-d', 'date.timezone=UTC',
            '-d', "include_path=$::imscpConfig{'SHARE_DIR'}/iMSCP/Package/Installer/Frontend/gui/library:/usr/share/php",
            "$::imscpConfig{'SBIN_DIR'}/imscp-update.php"
        ],
        \my $stdout,
        \my $stderr
    );
