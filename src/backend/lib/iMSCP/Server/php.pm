=head1 NAME
 ...
 iMSCP::Server::noserver - i-MSCP PHP server implementation
...
use iMSCP::Cwd '$CWD';
use iMSCP::Service;
use parent qw/ iMSCP::Common::Singleton iMSCP::Installer::AbstractActions /;
...
sub _disableUnusedPhpVersions
{
    ...
    for my $version ( $self->getAvailablePhpVersions() ) {
        ...
        if ( ref $httpd ne 'iMSCP::Server::httpd::apache_php_fpm' || $httpd->{'phpConfig'}->{'PHP_VERSION'} ne $version ) {
