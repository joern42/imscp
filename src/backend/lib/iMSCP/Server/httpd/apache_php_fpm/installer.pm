=head1 NAME
 ...
 iMSCP::Server::httpd::apache_php_fpm::installer - i-MSCP Apache2/PHP-FPM Server implementation
...
package iMSCP::Server::httpd::apache_php_fpm::installer;
...
use File::Basename;
use Boolean;
use Crypt qw/ ALNUM randomStr /;
use Debug qw/ debug error /;
use Dialog::InputValidation qw/ isOneOfStringsInList isStringNotInList /;
use Dir;
use File;
use Execute 'execute';
use Getopt;
...
use SystemGroup;
use SystemUser;
use httpd::apache_php_fpm;
use sqld;
use version;
use parent 'Common::Singleton';
...
=item _init( )
 ...
 Return iMSCP::Server::httpd::apache_php_fpm::installer
...
sub _init
{
    ...
    $self->{'httpd'} = iMSCP::Server::httpd::apache_php_fpm->getInstance();
...
sub _askForPhpConfigLevel
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'php', 'alternatives', 'all' ] ) || $value !~ /^per_(?:site|domain|user)$/ ) {
        ...
        ( my $rs, $value ) = $dialog->select( <<'EOF', \%choices, ( grep ( $value eq $_, keys %choices ) )[0] || 'per_site' );
...
sub _askForFpmListenMode
{
    ...
    if ( isOneOfStringsInList( iMSCP::Getopt->reconfigure, [ 'php', 'alternatives', 'all' ] ) || isStringNotInList( $value, 'tcp', 'uds' ) ) {
        ...
        ( my $rs, $value ) = $dialog->select( <<'EOF', \%choices, ( grep ( $value eq $_, keys %choices ) )[0] || 'uds' );
...
sub _copyDomainDisablePages
{
    ...
    iMSCP::Dir->new( dirname => "$::imscpConfig{'CONF_DIR'}/skel/domain_disabled_pages" )->rcopy(
        "$::imscpConfig{'SRV_DIR'}/domain_disabled_pages", { preserve => 'no' }
    );
}
...
sub _installLogrotate
{
    ...
    $rs ||= $self->{'httpd'}->buildConfFile( 'logrotate.conf', {}, { destination => '/etc/logrotate.d/apache2' } );
    ...
    if ( !$rs && version->parse( "$self->{'phpConfig'}->{'PHP_VERSION'}" ) < version->parse( '7.0' ) ) {
        ...
        $rs ||= $self->{'httpd'}->buildConfFile( "$self->{'phpCfgDir'}/fpm/logrotate.tpl", {}, { destination => '/etc/logrotate.d/php5-fpm' } );
...
sub _cleanup
{
    ...
    $rs = execute( "rm -f $::imscpConfig{'SRV_DIR'}/*/logs/*.log", \my $stdout, \my $stderr );
    debug( $stdout ) if $stdout;
    error( $stderr || 'Unknown error' ) if $rs;
    return $rs if $rs;
