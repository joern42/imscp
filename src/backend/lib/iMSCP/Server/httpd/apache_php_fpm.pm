=head1 NAME
 ...
 iMSCP::Server::httpd::apache_php_fpm - i-MSCP Apache2/PHP-FPM Server implementation
...
package iMSCP::Server::httpd::apache_php_fpm;

use strict;
use warnings;
use autouse 'Date::Format' => 'time2str';
use Class::Autouse qw/ :nostat iMSCP::Server::httpd::apache_php_fpm::installer iMSCP::Server::httpd::apache_php_fpm::uninstaller /;
use File::Basename;
use File::Spec;
use File::Temp;
use iMSCP::Boolean;
...
sub registerInstallerDialogs
{
    ...
    iMSCP::Server::httpd::apache_php_fpm::installer->getInstance()->registerInstallerDialogs( $dialogs );
}
...
sub install
{
    ...
    $rs ||= iMSCP::Server::httpd::apache_php_fpm::installer->getInstance()->install();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Server::httpd::apache_php_fpm::uninstaller->getInstance()->uninstall();
...
sub setEnginePermissions
{
    ...
    $rs ||= setRights( "$::imscpConfig{'SRV_DIR'}/domain_disabled_pages", {
        user      => $::imscpConfig{'ROOT_USER'},
...
sub deleteDmn
{
    ...
    unless ( $data->{'SHARED_MOUNT_POINT'} || !-d $data->{'WEB_DIR'} ) {
        ( my $userWebDir = $::imscpConfig{'SRV_DIR'} ) =~ s%/+$%%;
...
=item _init( )
 ...
 Return iMSCP::Server::httpd::apache_php_fpm
...
sub _addFiles
{
    ...
    if ( $data->{'WEB_FOLDER_PROTECTION'} eq 'yes' ) {
        my $dir = $data->{'WEB_DIR'};
        my $userWebDir = File::Spec->canonpath( $::imscpConfig{'SRV_DIR'} );
        do {
            setImmutable( $dir );
