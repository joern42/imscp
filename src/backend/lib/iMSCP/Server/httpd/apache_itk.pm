=head1 NAME
 ...
 iMSCP::Server::httpd::apache_itk - i-MSCP Apache2/ITK Server implementation
...
package iMSCP::Server::httpd::apache_itk;

use strict;
use warnings;
use autouse 'Date::Format' => 'time2str';
use Class::Autouse qw/ :nostat iMSCP::Server::httpd::apache_itk::installer iMSCP::Server::httpd::apache_itk::uninstaller /;
use File::Basename;
use File::Spec;
use File::Temp;
use iMSCP::Boolean;
...
sub registerInstallerDialogs
{
    ...
    iMSCP::Server::httpd::apache_itk::installer->getInstance()->registerInstallerDialogs( $dialogs );
}
...
sub install
{
    ...
    $rs ||= $rs = iMSCP::Server::httpd::apache_itk::installer->getInstance()->install();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Server::httpd::apache_itk::uninstaller->getInstance()->uninstall();
    ...
    unless ( $rs || !iMSCP::Service->getInstance()->hasService( $self->{'config'}->{'HTTPD_SNAME'} ) ) {
        ...
        @{ $self }{qw/ start restat /} = ( FALSE, FALSE );
    }
...
=item _init( )
 ...
 Return iMSCP::Server::httpd::apache_itk
