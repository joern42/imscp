=head1 NAME
 ...
 iMSCP::Server::po::courier - i-MSCP Courier IMAP/POP3 Server implementation
...
package iMSCP::Server::po::courier;

use strict;
use warnings;
use Class::Autouse qw/ :nostat iMSCP::Server::po::courier::installer iMSCP::Server::po::courier::uninstaller /;
use File::Temp;
use iMSCP::Boolean;
use iMSCP::Config;
...
sub registerInstallerDialogs
{
    ...
    iMSCP::Server::po::courier::installer->getInstance()->registerInstallerDialogs( $dialogs );
}
...
sub install
{
    ...
    $rs ||= iMSCP::Server::po::courier::installer->getInstance()->install();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Server::po::courier::uninstaller->getInstance()->uninstall();
...
sub addMail
{
    ...
    if ( $data->{'MAIL_QUOTA'} ) {
        if ( $self->{'forceMailboxesQuotaRecalc'} || ( iMSCP::Getopt->context() eq 'backend' && $data->{'STATUS'} eq 'tochange' )
...
=item _init( )
 ...
 Return iMSCP::Server::po::courier
...
sub _init
{
    ...
    $self->_mergeConfig() if iMSCP::Getopt->context() eq 'installer' && -f "$self->{'cfgDir'}/courier.data.dist";
    tie %{ $self->{'config'} },
        'iMSCP::Config',
...
sub _mergeConfig
{
    ...
    if ( -f "$self->{'cfgDir'}/courier.data" ) {
        ...
        tie my %oldConfig, 'iMSCP::Config', fileName => "$self->{'cfgDir'}/courier.data", readonly => TRUE;
