=head1 NAME
 ...
 iMSCP::Server::po::dovecot - i-MSCP Dovecot IMAP/POP3 Server implementation
...
package iMSCP::Server::po::dovecot;

use strict;
use warnings;
use Class::Autouse qw/ :nostat iMSCP::Server::po::dovecot::installer iMSCP::Server::po::dovecot::uninstaller /;
use File::Temp;
use iMSCP::Boolean;
use iMSCP::Config;
...
sub registerInstallerDialogs
{
    ...
    iMSCP::Server::po::dovecot::installer->getInstance()->registerInstallerDialogs( $dialogs );
}
...
sub install
{
    ...
    $rs ||= iMSCP::Server::po::dovecot::installer->getInstance()->install();
...
sub uninstall
{
    ...
    $rs ||= iMSCP::Server::po::dovecot::uninstaller->getInstance()->uninstall();
...
sub setEnginePermissions
{
    ...
    $rs ||= setRights( "$::imscpConfig{'ENGINE_ROOT_DIR'}/quota/imscp-dovecot-quota.sh", {
...
=item _init( )
 ...
 Return iMSCP::Server::po::dovecot
...
sub _init
{
    ...
    $self->_mergeConfig() if iMSCP::Getopt->context() eq 'installer' && -f "$self->{'cfgDir'}/dovecot.data.dist";
    tie %{ $self->{'config'} },
        'iMSCP::Config',
