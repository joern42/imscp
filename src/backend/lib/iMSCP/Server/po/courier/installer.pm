=head1 NAME
 ...
 iMSCP::Server::po::courier::installer - i-MSCP Courier IMAP/POP3 Server installer implementation
...
package iMSCP::Server::po::courier::installer;
...
use iMSCP::InputValidation qw/ isStringInList isOneOfStringsInList isValidUsername isStringNotInList isValidPassword isAvailableSqlUser /;
use iMSCP::Dir;
...
use iMSCP::Server::mta::postfix;
use iMSCP::Server::po::courier;
...
=item _init( )
 ...
 Return iMSCP::Server::po::courier::installer
...
sub _init
{
    ...
    $self->{'po'} = iMSCP::Server::po::courier->getInstance();
    $self->{'eventManager'} = $self->{'po'}->{'eventManager'};
    $self->{'dbh'} = $self->{'po'}->{'dbh'};
    $self->{'mta'} = iMSCP::Server::mta::postfix->getInstance();
...
sub _buildConf
{
    ...
    if ( -f "$self->{'cfgDir'}/imapd.local" ) {
        ...
        replaceBlocByRef( qr/(:?^\n)?# iMSCP::Server::po::courier::installer - BEGIN\n/m, qr/# iMSCP::Server::po::courier::installer - ENDING\n/, '', \$fileC );

        $fileC .= <<"EOF";

# iMSCP::Server::po::courier::installer - BEGIN
. $self->{'cfgDir'}/imapd.local
# iMSCP::Server::po::courier::installer - ENDING
EOF
        $file->set( $fileC );
...
sub _buildDHparametersFile
{
    ...
    my $rs = step(
        sub {
            ...
            my $rs = executeNoWait( $cmd, ( iMSCP::Getopt->noninteractive && iMSCP::Getopt->verbose ? undef : $outputHandler ), $outputHandler );
...
sub _migrateFromDovecot
{
    ...
    $rs = execute(
        [
            'perl', "$::imscpConfig{'SBIN_DIR'}/vendor/courier-dovecot-migrate.pl", '--to-courier', '--quiet', '--convert',
    ...
    unless ( $rs ) {
        ...
        $::imscpOldConfig{'PO_PACKAGE'} = 'iMSCP::Server::po::courier';
    }
