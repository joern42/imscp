...
if ( $db_pass_key eq '{KEY}' || $db_pass_iv eq '{IV}' ) {
    print STDERR( "Key file not found at $::cfg{'CONF_DIR'}/imscp-db-keys. Rerun the i-MSCP installer to fix." );
    exit 1;
}

$::db_pass_key = $db_pass_key;
$::db_pass_iv = $db_pass_iv;
...
$::imscp_pkt_mngr = "$::root_dir/engine/bin/imscp-pkt-mngr";
...
$::imscp_sw_mngr = "$::root_dir/engine/bin/imscp-sw-mngr";
