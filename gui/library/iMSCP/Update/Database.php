...
class iMSCP_Update_Database extends iMSCP_Update
{
    ...
    /**
     * Prohibit upgrade from i-MSCP versions older than 1.1.0
     *
    ...
    protected function r173()
    {
        throw new RuntimeException(
            'Database upgrade from an i-MSCP versions older than 1.1.0 is not supported. Please upgrade first to i-MSCP version 1.3.8.'
        );
    }
    ...
    protected function r203()
    {
        return [
            $this->changeColumn(
                'domain', 'allowbackup', "allowbackup varchar(12) COLLATE utf8_unicode_ci NOT NULL DEFAULT 'dmn|sql|mail'"
            ),
    ...
    protected function r225()
    {
        ...
        $sql = $this->addColumn(
            'subdomain', 'subdomain_type_forward', "VARCHAR(5) COLLATE utf8_unicode_ci DEFAULT NULL AFTER subdomain_url_forward"
        );
        ...
        try {
            ...
            while ($reseller = $resellers->fetchRow()) {
                ...
                while ($client = $clients->fetchRow()) {
                    ...
                    $domain = exec_query("SELECT domain_id FROM domain WHERE domain_admin_id = ? AND domain_status <> ?", [
     ...
     *
     * Note: Repeated update due to mistake in previous implementation (was r234 and r262, r267)
     *
     * @return array SQL statements to be executed
     */
    protected function r271()
    {
        try {
            ...
            if ($statements = $this->dropIndexByColumn('php_ini', 'admin_id')) {
                foreach ($statements as $statement) {
                    exec_query($statement);
                }
            }

            if ($statements = $this->dropIndexByColumn('php_ini', 'domain_id')) {
                foreach ($statements as $statement) {
                    exec_query($statement);
                }
            }

            if ($statements = $this->dropIndexByColumn('php_ini', 'domain_type')) {
                foreach ($statements as $statement) {
                    exec_query($statement);
                }
            }
     ...
     *   - Add compound PRIMARY KEY (domain_id, dtraff_time)
     *
     * @return null|string SQL statement to be executed
     */
