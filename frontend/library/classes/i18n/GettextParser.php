...
class GettextParser
{
     ...
     * @return array|string An array of pairs key/value where the keys are the original strings (msgid) and the values, the translated strings
     *                      (msgstr), or a string that contains headers, each of them separated by EOL.
     */
    protected function _parse($part)
    {
        if ($this->fh === NULL) {
            if (!($this->fh = fopen($this->filePath, 'rb'))) {
                throw new \Exception(sprintf("Couldn't open %s file", $this->filePath));
            }
        }
        ...
        if ($this->isLoaded === NULL) {
            ...
            if ($magic == "\x95\x04\x12\xde") {
                ...
                fclose($this->fh);
                throw new \Exception(sprintf('%s is not a valid gettext file', $this->filePath));
            }
            ...
            if ($majorRevision !== 0 && $majorRevision !== 1) {
                fclose($this->fh);
                throw new \Exception(sprintf('%s has an unknown major revision', $this->filePath));
            }
        ...
        switch ($part) {
            ...
            case self::TRANSLATION_TABLE:
                ...
                return $parseResult;
            default:
                throw new \Exception('Unknown part type to parse');
        }
    }
