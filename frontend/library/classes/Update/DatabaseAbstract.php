 ...
 */
abstract class DatabaseAbstract extends UpdateAbstract
{
    /**
            ...
            try {
                ...
                foreach ($queries as $query) {
                    if (!empty($query)) {
                        ...
                        $stmt = $db->createStatement($query)->execute()->getResource();
                ...
                if ($db->getDriver()->getConnection()->getResource()->inTransaction()) {
                    $db->getDriver()->getConnection()->commit();
                }
            } catch (\Exception $e) {
                # Make sure that we are still in transaction due to possible implicite commit
                # See https://dev.mysql.com/doc/refman/5.7/en/implicit-commit.html
                if ($db->getDriver()->getConnection()) {
    ...
    public function __call($updateMethod, $params)
    {
        if (!preg_match('/^r[0-9]+$/', $updateMethod)) {
            throw new \Exception(sprintf('%s is not a valid database update method', $updateMethod));
        }
