...
class Net
{
    ...
    protected function extractIpAddresses()
    {
        ...
        foreach ($output as $line) {
            if (preg_match(
                ...
                $this->ipAddresses[static::compress($matches[3])] = [
                    'device'        => $matches[1],
     ...
     * @return mixed
     */
    public static function compress($ipAddr) 
    {
        if (static::getVersion($ipAddr) == 4) {
            return $ipAddr;
        }
     ...
     */
    public static function getVersion($ipAddr)
    {
     ...
     * @return string
     */
    public static function expand($ipAddr)
    {
        if (static::getVersion($ipAddr) == 4) {
            return $ipAddr;
        }
