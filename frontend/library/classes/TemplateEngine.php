...
class TemplateEngine
{
     ...
     * @param EventManagerInterface|NULL $em
     * @param StorageInterface|NULL $cache
     */
    public function __construct($tpldir = NULL, EventManagerInterface $em = NULL, StorageInterface $cache = NULL)
    {
        $this->tplDir = normalizePath($tpldir ?: Application::getInstance()->getConfig()['ROOT_TEMPLATE_PATH']);
        $this->em = $em ?: Application::getInstance()->getEventManager();
        $this->cache = $cache ?: Application::getInstance()->getCache();
    }
     ...
     *
     * @param string|array $tname Template name or template block name, or an array where keys are temlate names or template block names, and values
     *                            are the template file paths or names of the templates containing the template block
     * @param string $tvalue A temlate file path or name of template containing template block. Only relevant if $tname is not an array
     * @return void
     */
     ...
     *
     * @param string|array $tname Template name or template block name, or an array where keys are temlate names or template block names, and values
     *                            are the inline templates or names of the templates containing the template block
     * @param string $tvalue An inline temlate or name of template containing template block. Only relevant if $tname is not an array
     * @return void
     */
     ...
     *
     * A template variable, unlike a runtime template variable, is a variable that is defined in a template. Template variables are defined
     ...
     *
     * A runtime template variable, unlike a template variable, is a variable that has not been defined in a template. Those variables are result of
     ...
     * runtime variable
     *
     * @param string $varname Name of variable to which parse result will be assigned, or added
     * @param string $tname Name of template or block to parse
     */
    public function parse($varname, $tname)
    {
        $this->em->trigger(Events::onParseTemplate, $this, [
            'varname' => $varname,
            'tname'   => $tname
        ]);
        ...
        if (!isset($this->resolvedTemplates[$tname]) && !isset($this->resolvedTemplates[$pname = $this->findParentTemplate($tname)])) {
    ...
    protected function resolveTemplate($tname)
    {
        ...
        if (NULL !== $data = $this->cache->getItem($id)) {
        ...
        while (strlen($tpl) > $startPos) {
            ...
            if ($m['tagName'] != $beginTagName) {
                throw new \LogicException(sprintf('Block tag mismatch in the `%s` template:: (%s vs %s).', $tname, $beginTagName, $m['tagName']));
            }
    ...
    protected function loadTemplateFile($fpath)
    {
        ...
        $this->em->trigger(Events::onBeforeLoadTemplateFile, $this, [
            'context'      => $this,
            'templatePath' => $fpath
        ]);
        ...
        if ($fContent == '') {
            ...
            throw new \RuntimeException(sprintf(
                "The %s template file couldn't be loaded: %s -- File %s -- Line %d", $fpath, $error['message'], $error['file'], $error['line']
            ));
        }
    ...
    public function replaceLastParseResult($content, $varname = NULL)
    {
        ...
        if (!isset($this->tplRuntimeVariables[$varname])) {
            throw new \InvalidArgumentException(sprintf('Unknown `%s` runtime template variable. Did you forgot to call parse()?', $varname));
        }
