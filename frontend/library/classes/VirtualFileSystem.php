...
class VirtualFileSystem
{
    ...
    public function open()
    {
        ...
        if (Application::getInstance()->getConfig()['SERVICES_SSL_ENABLED'] == 'yes') {
        ...
        if (strpos(Application::getInstance()->getConfig()['iMSCP::Servers::Ftpd'], '::Vsftpd::') === FALSE && !@ftp_pasv($this->stream, true)) {
    ...
    protected function createFtpUser()
    {
        try {
            ...
            execQuery('INSERT INTO ftp_users (userid, passwd, uid, gid, shell, homedir, status) VALUES (?, ?, ?, ?, ?, ?, ?)', [
                ...
                $row['admin_sys_gid'],
                '/bin/sh',
                normalizePath(Application::getInstance()->getConfig()['USER_WEB_DIR'] . '/' . $this->user), 'ok'
            ]);
            execQuery('INSERT INTO ftp_group (groupname, gid, members) VALUES (?,?,?)', ['imscp_frontend', $row['admin_sys_gid'], $this->user]);
        } catch (\Exception $e) {
            if ($e instanceof \Exception && $e->getCode() == 23000) {
    ...
    public function put($file, $content, $transferMode = self::VFS_BINARY)
    {
        ...
        $tmpFile = @tempnam(Application::getInstance()->getConfig()['FRONTEND_ROOT_DIR'] . '/data/tmp', 'vfs_');
        if ($tmpFile === false) {
