...
if ($use_webdepot) {
    ...
    if (isset($_POST['uaction']) && $_POST['uaction'] == 'updatewebdepot') {
        ...
        if ($xml_file === false) {
            setPageMessage(tr("Unable to read Web software repository index file."), 'error');
            $error = 1;
        }
...
if (isset($_POST['upload']) && $_SESSION['software_upload_token'] == $_POST['send_software_upload_token']) {
    ...
    if ($_FILES['sw_file']['name'] != '' && !empty($_POST['sw_wget'])) {
        setPageMessage(tr('You have to choose between file-upload and wget-function.'), 'error');
        $success = 0;
    } elseif ($_FILES['sw_file']['name'] == '' && empty($_POST['sw_wget'])) {
        setPageMessage(tr('You must select a file to upload/download.'), 'error');
        $success = 0;
    } else {
        if ($_FILES['sw_file']['name'] && $_FILES['sw_file']['name'] != "none") {
            if (substr($_FILES['sw_file']['name'], -7) != '.tar.gz') {
                setPageMessage(tr("File needs to be a 'tar.gz' archive."), 'error');
                $success = 0;
            }
            $file = 0;
        } else {
    ...
    if ($success == 1) {
        ...
        execQuery(
            ...
            ",
            [
                $user_id, 'waiting_for_input', 'waiting_for_input', 'waiting_for_input', 'waiting_for_input', 0, $filename, 'waiting_for_input',
                'waiting_for_input', 'waiting_for_input', 'waiting_for_input', 1, 'toadd', 'yes'
            ]
        );
        ...
        if ($file == 0) {
            $dest_dir = $cfg['FRONTEND_ROOT_DIR'] . '/data/persistent/softwares/sw_depot/' . $filename . '-' . $sw_id . $extension;

            if (!is_dir($cfg['FRONTEND_ROOT_DIR'] . '/data/persistent/softwares/sw_depot/')) {
                @mkdir($cfg['FRONTEND_ROOT_DIR'] . '/data/persistent/softwares/sw_depot/', 0755, true);
            }

            if (!move_uploaded_file($_FILES['sw_file']['tmp_name'], $dest_dir)) {
                // Delete software entry
                execQuery('DELETE FROM web_software WHERE software_id = ?', [$sw_id]);
                $sw_wget = '';
                setPageMessage(tr('Unable to upload file. Max. upload filesize (%1$d MB) reached?', ini_get('upload_max_filesize')), 'error');
                $upload = 0;
            }
        }
        ...
        if ($file == 1) {
            ...
            if ($connection) {
                ...
                if ($remote_file_size < 1) {
                    ...
                    setPageMessage(tr('The remote filesize (%s) is lower than 1 byte. Please check the URL.', $show_remote_file_size), 'error');
                    $upload = 0;
                } elseif ($remote_file_size > $cfg['APS_MAX_REMOTE_FILESIZE']) {
                    // Delete software entry
                    execQuery('DELETE FROM web_software WHERE software_id = ?', [$sw_id]);
                    $show_max_remote_filesize = bytesHuman($cfg['APS_MAX_REMOTE_FILESIZE']);
                    setPageMessage(tr('Max. remote filesize (%s) is reached. Your remote file is %s', $show_max_remote_filesize, $show_remote_file_size), 'error');
                    $upload = 0;
                } else {
                    ...
                    if ($remote_file) {
                        ...
                        // Delete software entry
                        execQuery('DELETE FROM web_software WHERE software_id = ?', [$sw_id]);
                        setPageMessage(tr('Error: Remote File not found!'), 'error');
                        $upload = 0;
                    }
                }
            } else {
                // Delete software entry
                execQuery('DELETE FROM web_software WHERE software_id = ?', [$sw_id]);
                setPageMessage(tr('Could not upload file. File not found.'), 'error');
                $upload = 0;
            }
        }
        ...
        if ($upload == 1) {
            $tpl->assign(['VAL_WGET' => '']);
            sendDaemonRequest();
            setPageMessage(tr('File has been successfully uploaded.'), 'success');
        } else {
