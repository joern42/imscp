...
/**
 * Send domain order email
 *
 * @param  string $aliasName
 * @return bool TRUE on success, FALSE on failure
 */
function send_domain_order_email($aliasName)
{
    ...
    if (!$ret) {
        writeLog(sprintf("Couldn't send domain order email to %s", $row['admin_name']), E_USER_ERROR);
        return false;
    }
...
function addDomainAlias()
{
    ...
    if ($domainAliasName == '') {
        ...
        if (!validateDomainName($domainAliasName)) {
            ...
            $ret = false;
        } elseif (isWebDomainKnown($domainAliasName, Application::getInstance()->getSession()['user_created_by'])) {
    ...
    $isSuIdentity = $identity instanceof CpSuIdentityInterface;
...
Counting::userHasFeature('domain_aliases') or View::showBadRequestErrorPage();
