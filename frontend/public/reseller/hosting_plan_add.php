...
function checkInputData()
{
    ...
    if (!resellerHasFeature('subdomains')) {
        ...
        setPageMessage(tr('Incorrect subdomains limit.'), 'error');
        $errFieldsStack[] = 'sub';
    }
    ...
    if (!resellerHasFeature('mail')) {
        ...
        setPageMessage(tr('Incorrect mail accounts limit.'), 'error');
        $errFieldsStack[] = 'mail';
    }
    ...
    if (!resellerHasFeature('sql_db')) {
        ...
        setPageMessage(tr('SQL user limit is disabled.'), 'error');
        $errFieldsStack[] = 'sql_db';
    ...
    if (!resellerHasFeature('sql_user')) {
        ...
        setPageMessage(tr('Incorrect SQL users limit.'), 'error');
        $errFieldsStack[] = 'sql_user';
    } elseif ($sqlu == -1 && $sqld != -1) {
        setPageMessage(tr('SQL databases limit is not disabled.'), 'error');
        $errFieldsStack[] = 'sql_user';
        $errFieldsStack[] = 'sql_db';
    }
...
function addHostingPlan()
{
    ...
    $stmt = execQuery('SELECT id FROM hosting_plans WHERE name = ? AND reseller_id = ? LIMIT 1', [$name, $_SESSION['user_id']]);

    if ($stmt->rowCount()) {
        setPageMessage(tr('A hosting plan with same name already exists.'), 'error');
        return false;
    }
...
if (!empty($_POST) && checkInputData() && addHostingPlan()) {
    setPageMessage(tr('Hosting plan successfully created.'), 'success');
